...
function Row(props) {
  ...
  useEffect(() => {
    if (!orderProducts) {
      axios
        .get(`https://apivivero.herokuapp.com/orders/${row.id}`)
        .then((res) => {
                    ...
                    orderProducts.map((product) => (
                      <TableRow key={product.id}>
                        ...
                        <TableCell align="right">
                          {product.productsorder.quantity}
                        </TableCell>
                        <TableCell align="right">
                          {Math.round(
                            product.productsorder.price *
                              product.productsorder.quantity
                          )}
                        </TableCell>
                      </TableRow>
  ...
  return (
    <TableContainer component={Paper}>
      <Table aria-label="collapsible table">
        ...
        <TableBody>
          {orders && orders.map((order) => <Row key={order.id} row={order} />)}
        </TableBody>
      </Table>
    </TableContainer>
  );
...
export default function Me() {
  ...
  useEffect(() => {
    if (!orders) {
      if (user) {
        axios
          .get(`https://apivivero.herokuapp.com/users/${user.id}/orders`)
          .then((res) => {
            console.log(res.data);
