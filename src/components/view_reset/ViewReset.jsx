...
export default function ViewReset({ id, onClose }) {
  ...
  const onSubmit = function (event) {
    ...
    axios.post(`https://apivivero.herokuapp.com/reset`, data).then((res) => {
  ...
  return (
    <div className={s.ViewUser}>
      <div className={s.content}>
        ...
        <div>
          {" "}
          Nombre: <span>{input.name}</span>{" "}
        </div>
        <div>
          <button onClick={() => onClose(false)} className={[s.btn].join(" ")}>
            Salir
          </button>
        </div>
        <div>
          <button onClick={onSubmit} className={[s.btn].join(" ")}>
            Reset Password
          </button>
        </div>
      </div>
